...
function renderMenu(items, x, y) {
  ...
  ul.style.top =
    Math.max(
      Math.min(y, window.innerHeight + window.scrollY - ul.clientHeight),
      0,
    ) + "px";
...
function getDropTarget(event) {
  ...
  if (
    ...
    while (target && target.className != "column") target = target.parentNode; // target column
...
function isAbove(pageY, target) {
  return (
    pageY - window.scrollY - target.getBoundingClientRect().top >
...
function toggle(node, a) {
  ...
  if (isopen) {
    ...
    // open folder
    if (a.nextSibling) animate(node, a, isopen);
    else
      getChildrenFunction(node)(function (result) {
...
function animate(node, a, isopen) {
  ...
  var duration = scale(getConfig("slide"), 0.2, 1) * 1000;
  a.animationHandle = setTimeout(function () {
    a.animationHandle = null;
    if (isopen) a.parentNode.removeChild(wrap);
    else {
      wrap.className = null;
...
function getConfig(key) {
  ...
  else return theme.hasOwnProperty(key) ? theme[key] : config[key];
}
...
function setConfig(key, value) {
  if (value != null)
    localStorage.setItem(
      "options." + key,
      typeof config[key] === "number" ? Number(value) : value,
    );
  else {
  ...
  switch (key) {
    ...
    case "shadow_color":
      return (
        ...
        scale(getConfig("shadow_blur"), 7, 100) +
        "px " +
        value +
        "; }"
      );
    ...
    case "spacing":
      return (
        "#main a { line-height: " +
        scale(value, 2, 5.6, 0.8) +
        "; " +
        "padding-left: " +
        scale(value, 0.8, 2, 0.4) +
        "em; " +
        "padding-right: " +
        scale(value, 0.8, 2, 0.4) +
        "em; }"
    ...
    case "h_pos":
      ...
      return (
        "#main { left: " + scale(value, 0, margin / 2, -margin / 2) + "%; }"
      );
    case "v_margin":
      return (
        "#main { margin-top: " +
...
function onChange(key, value) {
  ...
  if (value != config[key]) {
    ...
    if (css) {
      var style;
      if (styles.hasOwnProperty(key)) style = styles[key];
      else {
      ...
      if (index === nav.children.length - 2) {
        ...
        imports.onchange = function () {
          try {
            ...
            imports.value = "";
            imports.placeholder =
              "Import error! Please check if your settings are valid JSON.";
          }
        };
      }
      return false;
